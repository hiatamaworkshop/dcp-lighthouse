...
## 2026-08-22 — RC dashboard 実地検証: 粗窓dip明滅は単発実測で改善傾向、別の潜在バグを1件発見

`referenceUsable` 床の締め直し (直前の (7)) 後、2026-07-25 finding「粗窓 dip タイルが
24〜38秒遅れて出現し 2.0〜2.3σ 境界で明滅する」が改善したかを `run-lighthouse` スキル手順で
実地検証した (`/events/snapshot` SSE を購読しつつ RC シナリオを実行)。

### 粗窓dipの実測 (単発、サンプル数1)

クリーンな単発実行 (サーバ再起動直後・`/demo/start?scenario=RC` を1回だけ呼ぶ) で:

- t=+18.2s に dip 2.6σ で出現、t=+28.2s に 2.8σ へ微増、以降 t=+37.3s まで**同じσ帯を維持**
- t=+38.3s にきれいに `baseline` のみへ復帰。**2.0σ境界での出入り (明滅) は一切観測されず**

07-25 finding が記録した「2.0〜2.3σ 境界の明滅」は今回は再現しなかった。ただし RC のバースト・
回復タイミングはシード無しの乱数依存で試行ごとにぶれる (実際、直前の別試行では dip が一度も
出ず、代わりに回復後に spike 2.4σ が出た回もあった) ため、**これは「改善した」という確証ではなく
単発実測の観察に留まる**。次に確度を上げるなら複数試行 (最低5〜10回) の再現率を取るべきだが、
それは今回のスコープ外 (「軽量な確認」から逸脱するため見送り)。07-25 finding 自体は残したまま、
本エントリを追記する形で記録する。

### 副産物で見つけた潜在バグ (未調査、要追跡)

検証中に `/demo/stop` → 間を置かず `/demo/start` を連続で呼ぶ手順を踏んだところ、以後の
`/events/snapshot` が `agents: []` / `globalStats: {count:0, mean:0}` のまま固まり続ける状態を
1回再現した (`domains` の coverage も全ドメイン0%に張り付いた)。切り分けた結果:

- サーバをフレッシュ起動した直後 (scenario 未起動) は正常 (`agents:4`)
- `/demo/start` を1回だけ呼び、自然に (RC は約60秒で) auto-stop するのを待った後も正常
- **`/demo/stop` の直後に `/demo/start` を重ねた場合のみ**空の状態が固着した

`referenceUsable` の変更やこのセッションの他の修正とは無関係な、既存の生成器/収集器側の
挙動と見られる。stop/start の競合状態 (前のシナリオの後始末が終わる前に次が走り出す、
Step 3b で言う「動的データ追加」寄りの配線の穴の可能性) が疑わしいが未確認。
このエントリの時点では**原因調査に着手していない** — 次のタスクとして引き継ぐ。

### 追記: 原因特定・修正済み

`mock-stream-generator.ts` の `tick()` は `setInterval` 1本で駆動されており、
それを生成するのは **`index.ts` 起動時の `generator.start({rate:50})` 1回きり**。
`stop()` は `clearInterval` してタイマーを完全に破棄するが、**再始動する呼び出しがどこにも無い**。
`/demo/stop` を単独で呼ぶだけで (その後 `/demo/start` を呼ばなくても) 再現することを確認 —
`/demo/start` 側の競合ではなく、`stop()` 単体の設計漏れだった。`/status` が返す
`eventsPerSec:50` は静的な設定値であって実測ではないため、この停止状態でも数値上は
気づけない (別の「盲目と静寂の混同」パターン)。CLAUDE.md の「ベースラインはシナリオ間も
流れ続ける」という設計原則にも反する。

**修正**: `dashboard.ts` の `/demo/start` ハンドラで、`runScenario` を呼ぶ前に
`this.generator.start()` を呼ぶ (タイマーが既に動いていれば `start()` 自体が no-op なので副作用なし)。
`stop()` の意味 (「generator を止める」) 自体は変えていない — 再始動する経路を足しただけ。
...
## 2026-08-22 (3) — L5 較正測定: 連続性補正は切れないが、疎化はタダではない

前段の予測 (「格子検出は加重で一般化済みなので連続性補正は切れない」) を実測で確認した。
**予測は当たったが、それだけでは「疎化はカロリーゼロ」ではない** — 実測して初めて分かった。

### やったこと

- `calibration.ts` に疎化ありでの測定を配線。2点、既存の figure に影響しないことを確認済み:
  - `buildTrial`/`CalibrationOptions` に `retention?: Partial<RetentionBufferOptions>` を追加。
    既存呼び出し (`retention` 省略) は `retentionWindowMs: spanMs*10` のまま=参照ゾーン発火せず、
    354件のバイト同一を確認してから次へ進んだ
  - `emit()` の push 順を「乱数で引いた順」から「ts昇順」に変更。`RetentionBuffer.evict()`
    は配列の先頭を最古と仮定して前から刈るため、これは push 順 ≈ ts 順を要求する暗黙の前提
    だった。`retentionWindowMs` がスパンの10倍だった間はエビクションが一度も走らず無害だったが、
    疎化較正で意図的に `retentionWindowMs` をスパンより小さくして参照ゾーンを稼働させると、
    この前提が初めて効いてくる。`segment()` 自体は返却時にソートするので既存345件の結果には
    無関係 (エビクションが起きない設定では push 順は最終結果に影響しない) — 実測でも347→347で
    変化なしを確認してから疎化較正へ進んだ
- テスト条件: `retentionWindowMs = spanMs` (参照スパン全体を鮮度ゾーンから追い出し、
  観測スパンは全解像のまま保つ — 疎化「だけ」の効果を切り分ける), `referenceWindowMs = spanMs*4`
  (盲目化しない余裕を確保)

### 測定結果 (SEEDS=500、shape既定: spanMs=10s, eventsPerSpan=1000, passRate=0.95)

| thinningRatio | 誤警報率 | 備考 |
|---|---|---|
| なし (無疎化) | 3.4% | 対照 |
| x2 (実効n≈500) | 4.0% | 設計値4.55%に近い |
| x5 (実効n≈200) | 9.0% | 悪化 |
| x10 (実効n≈100) | 11.5%(3件盲目) | さらに悪化 |
| x10, 密度4倍(実効n≈400) | 5.4% | **設計値近くまで回復** |

比較対照 (無疎化、eventsPerSpanを直接減らした場合): n=500→5.8%, n=200→7.6%, n=100→9.6%。
**疎化x10(実効n≈100)の11.5%は、無疎化でn=100まで薄めた9.6%とほぼ同じ桁**。

### 結論: 新しい機序ではなく、既知の「薄い窓」残差に合流するだけ

- 連続性補正自体は疎化でも切れていない (対照実験のx2は設計値近傍を維持、密度を戻すx10dense
  も設計値近傍に復帰) — 予測通り
- しかし**疎化は「実効サンプル数を減らす別の手段」であり、無料ではない**。1/N保持は
  参照側の実効n を eventsPerSpan/N まで落とし、これが decay の短いτや無疎化の低密度と
  **全く同じ「薄い窓」残差** (2026-08-17 較正較正で発見・記録済み、修正未完了の既知ギャップ) に
  合流する。疎化率を上げるほど、この既知の残差に近づいていくだけで、疎化固有の新しい
  誤較正メカニズムは見つからなかった
- power(検出力)もx2で使用感が保たれること (44.0%→40.2%、diff<0.08の帯域内) を確認。
  x10側は明示的な power 測定はテストに含めていない (誤警報側が既にthreshold超過を示しており、
  「薄い窓」残差の一種であることは density relief 実験で十分示せたため)
- テスト3件追加 (calibration.test.ts)。テスト 354→357件、`npx tsc` 通過、全 green
...
## 2026-08-22 (4) — L5 本番配線: `index.ts` に参照ゾーンを有効化

較正測定の結果 (x2=安全域、x5以降は既知の薄い窓残差) を踏まえて数値を決め、配線した。
これで L5 (§B) の Step 1〜4 (weight plumbing → RetentionBuffer本体 → 較正測定 → 本番配線) が完了。

- `index.ts` に `REFERENCE_WINDOW_MS = RETENTION_WINDOW_MS * 10` (=1,200,000ms、鮮度ゾーン120秒の
  10倍、丸め数字であって測定値ではない) と `REFERENCE_THINNING_RATIO = 2` (**これは測定値** —
  calibration.test.ts が誤警報4.0%/設計4.55%・power帯域内と実測した比率をそのまま採用。
  x5以降を採用しなかった理由も含めてコード内コメントに較正結果を明記)
- `new RetentionBuffer(...)` にこの2値を渡すだけ。`RetentionBuffer`/`lens.ts` 側は前段で
  実装済みなのでロジック変更は無し
- `$Q` 経由の動的再設定は依然未配線 (`RetentionBuffer` に `setReferenceWindowMs`/
  `setThinningRatio` が無い — `retention_window_ms` 同様の setter がまだ無いため、
  `q-retention-binding.ts` を拡張しても紐づけ先が無い)。今回は起動時定数のまま
- 実サーバでの疎通確認: 起動 → `/status` → `/demo/start?scenario=RC` → `/status`
  (`activeScenario:"RC"`) → `/demo/stop` まで例外なし。参照ゾーンが有効な状態での
  RC実行に構造的な問題が無いことを確認 (RCのreplay往復は120秒鮮度ゾーン内に収まる規模なので、
  この確認は「参照ゾーンがあっても既存動作を壊さない」ことの確認であって、
  「参照ゾーンが実際に使われる」実地確認ではない — 参照ゾーンを実際に読みに行くには
  120秒より過去を指すreplayリクエストが要る。それを起こす経路は今のところ無い)
- `npx tsc` 通過、`npm test` 357/357 green (ロジック変更なしなので件数は変化なし)
