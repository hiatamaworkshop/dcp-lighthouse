...
> **Third axis — silence vs blindness (field finding, real-data application).** The signature distinction "the world changed vs the lens changed" assumes events keep arriving; on a mock stream they always do. Applying the mechanism to a *real, sparse* transport (see ROADMAP_BRIEF "Field findings") surfaces a third thing the absence of events can mean: **the world is silent** (no activity — normal) vs **the observer is blind** (the transport is down — we stopped seeing). A coverage/gap signal that reads only event flow conflates the two and false-fires on normal quiet. The fix is to make **transport liveness a first-class observation input, separate from event flow**, so a gap is "blind" only when liveness says the channel is down. This generalizes the lighthouse distinction: the world changing, the lens changing, and now the observer's own sight failing are three independent axes the observation layer must keep apart.

> **Text as a stream — observe statistics, not content (field finding, second non-test domain).** A second application (the trading observatory's macro-context feed) needed to fold *text* — official announcements and social chatter — into the same observation layer. The instinct is to parse meaning per item, which drags in an LLM and inherits each item's unreliability. The lighthouse framing dissolves this: treat text as a `mention:v1`-style event stream and **observe the statistics of the stream (rate, shape), not the meaning of any item.** Light parsing extracts a few features (symbol match, cheap polarity proxy, source tag); the observation layer does the aggregation. Then the existing mechanics transfer wholesale — a **mention-rate spike is an AR-style anomaly**, and **silence vs blindness (the third axis above) applies directly**: zero mentions means a quiet world or a dead feed, told apart by transport liveness, not by content. Crucially, **this is trustworthy even when individual items are not**: a surge in mentions is a real, measurable event regardless of whether any single post is true — aggregate stream statistics are more reliable than any item. It confirms "the domain is a skin" on a stream type (unstructured text) far from the test-result origin: the observation layer never had to *understand* the text, only to *watch its shape*.
